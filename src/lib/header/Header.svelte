...
<nav class="bg-white border-gray-200 px-2 sm:px-4 py-2.5 rounded dark:bg-gray-900">
	<div class="container flex flex-wrap justify-between items-center mx-auto">
	  ...
	  <button on:click={showMenu} type="button" class="inline-flex items-center p-2 ml-3 text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-default" aria-expanded="false">
	  ...
	  <div class="{menu? 'block sm:hidden w-full' : 'hidden sm:hidden w-full'}" id="navbar-default">
		<ul class="flex flex-col p-4 mt-4 bg-gray-50 rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
		  ...
		  <li on:click={showMenu} class="{tab === "about" ? 'opal border-b-2 transition-all' : ''}">
		  ...
		  <li on:click={showMenu} class="{tab === "market" ? 'opal border-b-2 transition-all' : ''}">
		  ...
		  <li on:click={showMenu} class="{tab === "contact" ? 'opal border-b-2 pr-1 transition-all' : 'opal hover:border-b-2 transition-all'}">
			<a href="/contact" on:click={() => tab = "contact"} class="block py-2 pr-4 pl-3 text-gray-700 rounded md:p-0">Contact</a>
		  </li>
